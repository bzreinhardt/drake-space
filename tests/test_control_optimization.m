function [outputs,fval,exitflag,out] = test_control_optimization
...
out = cell(3,1);
for j = 1:3
    [outputs(j,:),fval(j),exitflag(j),out{j}] = fmincon(@(u)objective(u,j),1000*randn(1,4),[],[],[],[],LB*ones(1,4),UB*ones(1,4));
    while exitflag(j) == 2
        disp(strcat('exitflag = 2, rerunning for i = %n',num2str(j)))
        [outputs(j,:),fval(j),exitflag(j),out{j}] = fmincon(@(u)objective(u,j),1000*randn(1,4),[],[],[],[],LB*ones(1,4),UB*ones(1,4));
    end
end
