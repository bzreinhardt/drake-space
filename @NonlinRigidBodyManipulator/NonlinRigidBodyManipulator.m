classdef NonlinRigidBodyManipulator < NonlinManipulator
  ...
  methods
    function obj = NonlinRigidBodyManipulator(urdf_filename,options)
  ...
  methods
    ...
    function y = output(obj,t,x,u)
      ...
      if isempty(obj.sensor)
        y = x;
        if numel(x) ~= obj.num_positions + obj.num_velocities
            warning('Drake:RigidBodyManipulator:OutputStateNumber mismatch between number of output states and acutual system states');
        end
      else
    ...
    function B = getB(obj,q,qd)
        %TODO Mod this. See where it is called
    ...
    function obj = setJointLimits(obj,jl_min,jl_max)
      obj = setJointLimits@NonlinManipulator(obj,jl_min,jl_max);
      obj.dirty = true;
    end
    ...
    function [f,dfdq,dfdforce] = cartesianForceToSpatialForce(obj,kinsol,body_ind,point,force)
      % @param kinsol - solved kinematics for the body 
      % @param body_ind is an index of the body
      % @param point is a point on the rigid body (in body coords) 
      % @param force is a cartesion force (in world coords) (3 vector) 
    ...
    function model = compile(model)
      ...
      for i=1:length(model.force)
        if model.force{i}.direct_feedthrough_flag
          ...
          model.force{i} = model.force{i}.setInputNum(input_num); %increase input num
          u_limit(size(u_limit,1)+1,:) = model.force{i}.input_limits; %add another row to u_limit
        end
      end
      model.B = full(B);
...
%      node_names = regexprep({model.body.linkname},'+(.)*','');
    ...
    function v = constructVisualizer(obj,options)
        %Constructs an LCL visualization object
    ...
    function varargout = pdcontrol(sys,Kp,Kd,index)
      ...
      [varargout{:}] = pdcontrol@NonlinManipulator(sys,Kp,Kd,index);
    end
    ...
    function [xstar,ustar,success] = findFixedPoint(obj,x0,u0,options)
      ...
      function [c,ceq,GC,GCeq] = mycon(quz)
        ...
        c = [];
        GC = [];
        [~,C,B,~,dC,~] = obj.nonlinManipulatorDynamics(q,zeros(nv,1));
...
%      end
    ...
    function d=surfaceTangents(obj,normal)
      ...
      assert(~isempty(normal), ...
        'Drake:RigidBodyNonlinManipulator:surfaceTangents:emptyNormals', ...
...
%        children = find([model.body.parent] == body);
    ...
    function model = parseForceElement(model,robotnum,node,options)
      ...
      elnode = node.getElementsByTagName('induction_coupler').item(0);
      if ~isempty(elnode)
        [model,fe] = RigidBodyCoupler.parseURDFNode(model,robotnum,elnode,options);
      end
