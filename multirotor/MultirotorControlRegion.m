classdef MultirotorControlRegion < DrakeSystem
    properties
        c; %controller
    ...
    methods
        %Constructor
        function obj = MultirotorControlRegion(x0,sys)
%should check for x0 matching sys dimensions
            ...
            %find the possible fixed point nearest the state
            ...
            if (nargin > 1)
              ...
              lastwarn('');
            [xstar,ustar,info] = sys.findFixedPoint(x0,zeros(sys.getNumInputs,1));
            warn = lastwarn;
            if ~isempty(warn) > 0 && strcmp(warn(end-10:end),'minimized')
                error('MULTIROTORCONTROLREGION: failed to find a valid fixed state');
            else
                
                if ~isequal(xstar.double,x0)
                    warning('MULTIROTORCONTROLREGION: x0 not a stable point. switch to stable point');
                    disp('new x0 = ');
                    disp(xstar.double);
                end
                obj.x0 = xstar.double;
